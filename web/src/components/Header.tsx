        ...
        {options.map((o) => {
          ...
          return (
            <button key={o} onClick={() => toggle(o)}
              className={`rounded-full border px-2.5 py-0.5 text-xs transition ${on ? "border-risk-line bg-risk-bg text-risk-fg" : "border-line bg-surface text-faint line-through"}`}>
              {o}
            </button>
          );
        })}
...
export default function Header() {
  ...
  return (
    <header>
      ...
      <div className="mt-4 rounded-lg border border-line bg-surface p-4 space-y-3">
        <div className="grid grid-cols-[auto_1fr_1fr] gap-6">
          <div>
            <div className="mb-1 text-xs font-medium text-muted">Date range</div>
            <div className="flex items-center gap-1 text-sm">
              <input type="date" value={filters.start} min="2026-01-01" max="2026-05-03" onChange={(e) => setFilters({ ...filters, start: e.target.value })} className="rounded border border-line px-2 py-1" />
              <span className="text-faint">to</span>
              <input type="date" value={filters.end} min="2026-01-01" max="2026-05-03" onChange={(e) => setFilters({ ...filters, end: e.target.value })} className="rounded border border-line px-2 py-1" />
            </div>
          </div>
        ...
        <div className="text-xs text-faint">Click a chip to remove it from the filter. UNKNOWN = merchant not in master. NO_KYC_RECORD = user not in KYC file.</div>
      </div>
    </header>
  );
}
